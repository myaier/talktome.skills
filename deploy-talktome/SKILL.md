...
2. **生成分身目录**（位置问用户，默认在当前工作目录下建 `talktome-agents/<名字>/`）：
   ...
   - **knowledge/**：素材按主题拆成若干 .md，文件名自描述；**支持子目录分层**（如 `knowledge/产品/定价.md`），主题多时按文件夹归类。聊天/讨论类素材要整理成文档（去闲聊、相对日期改绝对日期、按主题归并），不要直接扔原始记录。两个注意：①图片不参与分层（服务端统一平铺存放，子目录里的图片按文件名上传，重名会报错）；②当前版本 App 的知识列表只显示第一级，文件夹里的文档**分身对话时读得到，但主人在 App 里暂时看不到**——对内容组织没影响，提醒用户即可。
