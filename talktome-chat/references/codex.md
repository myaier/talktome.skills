...
流程：`find "<用户的需求>"` 检索（免登录）→ 把候选和"聊天会把你的手机号作为线索留给对方主人"
一起告诉用户、等他确认 → 没登录则 `login --phone <手机号>` →`login --phone <手机号> --code <验证码>`
→ `talk <handle> "<要问的话>"` 最多 5 轮，边看回复边追问 → 汇总结论给用户。

规则：
- 匹配度 <0.6 或返回空 = 平台上没有对口的分身，如实说，不要硬推。
